...
const HorizontalForm = () => {
  ...
  const uploadImage = async () => {
    ...
    try {
      ...
      return data.url; // Assuming the API returns { url: "image_url" }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://carcarebaked.azurewebsites.net/api/createService', {
        method: 'POST',
        headers: {
    ...
    <Card>
      ...
      <CardBody>
        <Form onSubmit={handleSubmit}>
          ...
          <Row className="mb-1">
            <Label sm="3" for="vendorCommission">
              Vendor Commission
            </Label>
            <Col sm="9">
              <Input
